...
def medical_dashboard(request):
    medical = Medical.objects.get(user=request.user)

    total_patients = Appointment.objects.filter(medical=medical).values('patient').distinct().count()
    total_prescriptions = Prescription.objects.count()
    pending_bills = Appointment.objects.filter(medical=medical, p_status="pending").count()
    completed_bills = Appointment.objects.filter(medical=medical, p_status="complete").count()
    ...
    return render(request,'medical/medical_dashboard.html', d)
...
def d_appointment(request):
    if not access(request.user):
        ...
        return redirect('doctor_profile')
    data=Appointment.objects.filter(doctor=Doctor.objects.get(user=request.user),status="pending").order_by("-id")
    d={'data':data}
...
def patient_invoices(request,pid,task):
    ...
    bills = Billing_Record.objects.filter(appoint=pat)
    total = bills.aggregate(total_amount=Sum('amount'))['total_amount']
...
def edit_admin_profile(request):
    data = Adminstration.objects.filter(user=request.user).first()
    ...
    if request.method == "POST":
        ...
        if 'pwd2' in request.POST:
            ...
            confirm_password = request.POST.get('pwd3')
